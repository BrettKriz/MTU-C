...
int body(){
    ...
    do {
	   int ret = -1;
           //fread(&data[currentPosition], 1,1, f);
           ret = getNextBit();
	   if(ret != -1){
                data[currentPosition] = I2C(ret);
    ...
    */
    ...
    for(int x = 0; x < size; x++){
        ...
        if (count <= 0){
            ...
            printf("Real file size: %d", real_size);
            break;
        }else{
            //printf("Reading @ %d", x);
        }
    ...
    if (fread(&temp, 1, 1, f)){
        ...
        printf("The file exceeded the %f bit limit!",size*0.95);
        fclose(f);
        return EXIT_FAILURE;
    }
    //*/
    fclose(f);
    printf("\nFile closed! Processing...\n");
    /*
    printf("\n END \n");
    // *
    for (int i = 0; i < size; i++){
