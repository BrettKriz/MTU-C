...
public class ArraySequence<E> implements Sequence<E>
{
    ...
    private ArrayDNode[] elements = new ArrayDNode[1];
    ...
    *  DNode if it is valid; O(1) time */
    ...
    public int indexOf( Position<E> aPosition ) throws InvalidPositionException
    {
       ...
       checkIndex( arg.getIndex() , size());

        // Now make sure it is correct.
       Position<E> element = this.elements[ arg.getIndex() ];
        if ( element.equals( aPosition ) ) {
            return arg.getIndex();
        }else{
            throw new Error("<!> Arrays mixed up or misslabeld! Looking for :" + aPosition.element() + ": | Found different position :" + arg.element());
    ...
    private ArrayDNode[] resize(ArrayDNode[] elems, int newsize){
        if(newsize < 0){
    ...
    {
        return this.remove( this.atIndex(i) );
    }
    ...
    private void shiftDown(int at){
      ...
      // ASSUMES SIZE IS ALREADY SUBTRACTED
      for (int i = at; i < this.size() - 1; i++) {
        // shift elements down
    ...
    private void insert(int at, ArrayDNode<E> item){
        ...
        at = Math.max(at,0);
        int size = Math.abs(this.size()-at);
        checkSize( size );
        ...
        for (int i = this.size()-1; i >= at; i--) {
            ...
            if (this.elements[i + 1] != null){
                this.elements[i + 1].setIndex(i + 1);
            }
        }
    ...
    public boolean isFirst(Position<E> p)
      throws InvalidPositionException {  
      ArrayDNode<E> v = checkPosition(p);
      return header.getNext().equals(v);
    }
...
class ArrayDNode<E> implements Position<E> {
    ...
    @TimeComplexity("O(1)")
    public void NeighborCheck(ArrayDNode<E> prev, ArrayDNode<E> next){
        // Make sure we have good starting info
    ...
    public E element() throws InvalidPositionException {
      if ( (this.prev == null) && (this.next == null) ) 
            throw new InvalidPositionException("Position is not in a array!");
      return element;
    }
...
// ITERATORS
class SequenceIterable<E> implements Iterable<E>{
    private E[] array;
...
class SequenceIterator<E> implements Iterator<E> {
    ...
    public void remove(){
        throw new UnsupportedOperationException("No you.");
    }
