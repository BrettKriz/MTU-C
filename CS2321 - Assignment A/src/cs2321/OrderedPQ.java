...
public class OrderedPQ<K extends Comparable<K>,V> extends PriorityQueueBase<K,V> implements MinPriorityQueue<K,V> {
        ...
        @TimeComplexity("O(n)")
	public Entry<K,V> insert(Entry<K,V> var) throws InvalidKeyException {
            QEntry<K,V> arg = (QEntry)var;
            K key = arg.getKey();
            V value = arg.getValue();
	...
	public Entry<K,V> min() throws EmptyPriorityQueueException {
            int at = size()-1;
            return backing.get( at ); // backing.size() - 1
	}
	...
	public Entry<K,V> removeMin() throws EmptyPriorityQueueException {
            ...
            // so o(1) 
            QEntry<K,V> ans = backing.remove( size()-1 );
