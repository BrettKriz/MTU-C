...
public class ArraySequence<E> implements Sequence<E>
{
    ...
    *  DNode if it is valid; O(1) time */
    ...
    @TimeComplexity("O(n)") @TimeComplexityAmortized("O(1)")
    public void checkSize(int additive){
        //TCJ: The algorithim only has to resize on such intervals 
        // that its cost becomes O(1)
    ...
    @TimeComplexity("O(1)") 
    private void insertLast(ArrayDNode<E> item){
         // Shift the elements up
        
        int at = Math.max(this.numElts,0);
        item.setIndex(at);
