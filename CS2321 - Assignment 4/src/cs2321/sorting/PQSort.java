...
import cs2321.MaxPriorityQueue;
import cs2321.MinPriorityQueue;
//
import cs2321.UnorderedPQ;
import cs2321.OrderedPQ;
import cs2321.Heap;
import cs2321.CompleteTree;
...
public abstract class PQSort<K extends Comparable<K>>{
	...
	protected void sort(K[] kArray, MinPriorityQueue<K,K> pq) {
		// Get each value from the array, placing it into the Priority Queue
                //OrderedPQ<K,K> pq2 = (OrderedPQ<K,K>) pq;
		for(K k : kArray)
			pq.insert(k,k);
	...
	{
                //UnorderedPQ<K,K> pq2 = (UnorderedPQ<K,K>) pq;
		// Get each value from the array, placing it into the Priority Queue
		for(K k : kArray)
			pq.insert(k,k);
